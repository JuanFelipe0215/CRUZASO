...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  topbar.innerHTML = `
    <nav class="navbar bg-white border rounded-3 px-3 py-2 shadow-sm">
      <span class="fw-semibold">Task Form</span>
  ...
  if (taskId) {
    ...
    try {
      ...
      if (String(task.userId) !== String(session.id)) {
        alert("No puedes editar tareas que no son tuyas");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      console.error(err);
      showAlert("No se pudo guardar. Revisa JSON Server.", "danger");
    }
  });
});
